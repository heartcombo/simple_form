...
class SimpleFormGeneratorTest < Rails::Generators::TestCase
  ...
  test 'generates the simple_form initializer with the css-zero wrappers' do
    run_generator %w[--css-zero]
    assert_file 'config/initializers/simple_form.rb',
      /config\.default_wrapper = :default/, /config\.boolean_style = :nested/
    assert_file 'config/initializers/simple_form_css_zero.rb', /config\.wrappers :vertical_form/,
      /config\.default_wrapper = :vertical_form/, /config\.item_wrapper_tag = :div/
  end
