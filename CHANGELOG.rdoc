...
* enhancements
  ...
  * Add option and configuration to specify the collection wrapper class (github.com/mfila)
  * Add proc support to `collection` option (github.com/jeffkreeftmeijer)
  * `simple_form_for` allows default options for its inputs (:defaults => {})
  * Add `readonly` as option of input method (github.com/Untainted123)
  * `simple_fields_for` for inherits wrapper option form the form builder (github.com/nashby)
  * Deprecated part of the old configuration API in favor of the wrapper API which allows you to customize your inputs in a more flexible way. A guide on update is available here: https://github.com/plataformatec/simple_form/wiki/Upgrading-to-Simple-Form-2.0
  * Use action prefix in the form css class. Closes #360 (github.com/nashby)
...
* bug fix
  ...
  * Fix i18n lookup with attributes with same name of models. Closes #149 and #364 (github.com/nashby and github.com/MarceloCajueiro)
  * Do not generate `for` attribute for the collection label when it is a checkbox or radio. Closes #344 (github.com/nashby and github.com/mfila)
  * Select can have required option when the `:include_blank` option is passed. Closes #340 (github.com/nashby)
  * `:checked` option should override the existing associations on `collection_check_boxes`. Closes #341 (github.com/nashby)
